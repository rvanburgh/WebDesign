...
from flaskblog.models import User, Thought, Gratitude
...
def user_thoughts(name):
    page = request.args.get('page', 1, type=int)
    user = User.query.filter_by(name=name).first_or_404()
    thoughts = Thought.query.filter_by(author=user)\
        .order_by(Thought.date_posted.desc())\
        .paginate(page=page, per_page=5)
    return render_template('user_thoughts.html', thoughts=thoughts, user=user)

@users.route("/user/<string:name>")
def user_gratitudes(name):
    page = request.args.get('page', 1, type=int)
    user = User.query.filter_by(name=name).first_or_404()
    gratitudes = Gratitude.query.filter_by(author=user)\
